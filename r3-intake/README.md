...
- **Records event filter is strict attendance-only** (`handleList` in `internal/server/admin.go`):
  filtering the Records list by an event surfaces only intakes that have an attendance
  record for that event (`attendance.intake == intake.id`) whose `date` falls within
  the event's `start_date` -> `end_date` range. Attendance is the source of truth; an
  intake's home event (`intake.event`) never contributes. All attendance statuses count
  (present/absent/excused/walk_in). The filter is built as OR-joined
  `(id='<id1>' || id='<id2>' || ...)` clauses (PocketBase v0.39 has no `in` operator)
  and composes with the `?status=`/`?q=` filters via ` && `. An event with no in-range
  attendance records renders an empty list. The list no longer shows a per-row Event
  column; the event filter dropdown is the way to scope the list to an event.
