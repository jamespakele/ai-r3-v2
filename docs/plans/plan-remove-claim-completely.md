...
- After edits, `grep -rn "claimed\|assigned_to" r3-intake/internal r3-intake/cmd` must return ZERO hits. The only acceptable remaining references in the whole repo are in `pocketbase/migrations/` (001 history + 017).
...
- New migration test in `pocketbase/migrations/` (pattern: `014_users_deleted_test.go`): the harness boots PocketBase with the FULL migration chain already applied, so you cannot seed "before 017" through PocketBase saves. Working approach: after bootstrap, insert legacy-shaped rows with raw SQL (`app.DB().NewQuery` or sqlite3 against the temp data dir — bypasses schema validation), then call the 017 up function a second time (it must be idempotent) and assert: (a) no record has `status='claimed'`, (b) the intake collection has no `assigned_to` field, (c) the status select values are exactly `[unassigned completed]`. Include a down → up round-trip like 014 does.
- `claim_removal_integration_test.go`: `TestNewRecordNotAutoClaimed` reads `GetString("assigned_to")` — after the field is removed that read returns "", and the Story 2 grep gate flags the reference. Rewrite the assertion to check `status` + `created_by` only.
- Full gate green: `cd r3-intake && make build && go vet ./... && go test ./...`.
